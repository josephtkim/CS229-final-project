...
def evaluate_consistency(model, dataset, device, num_samples=100, threshold=0.20):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            # Re-check gender mismatch (just to be safe)
...
def visualize_consistency_pairs(model, dataset, device, num_samples=3, save_dir='eval_results'):
    ...
    with torch.no_grad():
        for i, (idx, mismatch_idx) in enumerate(zip(indices, mismatched_indices)):
            ...
            # Place main caption above the image
...
def visualize_image_to_text_generation(model, dataset, device, num_samples=4, save_dir='eval_results'):
    """
    Visualize image-to-text generation for multiple samples, saving a separate image per sample.

    For each randomly selected sample:
    - Shows the original image
    - Shows the model-generated text

    Saves each sample as a separate PNG file.
    """
    os.makedirs(save_dir, exist_ok=True)
    model.eval()
...
def visualize_text_to_image_generation(model, dataset, device, num_samples=4, save_dir='eval_results'):
    """
    Visualize text-to-image generation for multiple samples, saving a separate image per sample.
    
    For each randomly selected sample:
    - Shows the ground truth text (caption)
    - Shows the model-generated image from the attributes
